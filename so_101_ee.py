...
class SO101(BaseAgent):
	...
	keyframes = dict(
		...
		# 	qpos=np.array([0, -1.5708, 1.5708, 0.66, 0, 0.6]),
		# 	pose=sapien.Pose(),
		# ),
		rest=Keyframe(
			# qpos=np.array([0, 2.2, 3.017, -0.25, 0, 0.6044]),
			...
			# qpos=np.array([0, -0.45, 0.85, 1.0, 0, 0.6]),

			qpos=np.array([0, -0.8, 0.8, 0.9, 0, 0.6]),
			# qpos=np.array([0, -1.2, 1.4, -0.2, -np.pi/2, 0.0]),
	...
	def _controller_configs(self):
		...
		# ===== pd delta for gripper (one finger) =====
		gripper_pd_joint_delta_pos = PDJointPosControllerConfig(
			self.gripper_joint_names,
			[-0.4],
			[0.4],
			stiffness=[1e2], # smooth the gripper movement
			damping=[1e2],
			force_limit=100,
			use_delta=True,
			use_target=False,
		)
		gripper_pd_joint_target_delta_pos = copy.deepcopy(gripper_pd_joint_delta_pos)
		...
		arm_pd_ee_delta_pose = PDEEPoseControllerConfig(
			...
			rot_lower=-0.1,
			rot_upper=0.1,
			stiffness=[1e3] * len(self.arm_joint_names),
			damping=[1e2] * len(self.arm_joint_names),
			force_limit=100,
	...
	def _after_loading_articulation(self):
		...
		self.wrist_link = self.robot.links_map["wrist_link"]
