...
@dataclass
class Args:
	exp_name: Optional[str] = 'LiftCubeCamNewNet'
...
#     def __init__(self, sample_obs, feature_size=512):
#         super().__init__()
...
#             self.rgb_fc1 = nn.Linear(n_flatten, half_feature)
#             self.rgb_fc2 = nn.Linear(n_flatten, half_feature)
			
#             self.out_features += (half_feature * 2)

#         if "state" in sample_obs:
#             self.state_dim = sample_obs["state"].shape[-1]
#             self.extractors["state"] = nn.Sequential(
...
#             # 转换维度并归一化: [B, H, W, 6] -> [B, 6, H, W]
#             obs_rgb = observations["rgb"].float().permute(0, 3, 1, 2) / 255.0
			
#             rgb_v1 = obs_rgb[:, :3, :, :]
#             rgb_v2 = obs_rgb[:, 3:, :, :]
			
#             feat_v1 = F.relu(self.rgb_fc1(self.cnn_v1(rgb_v1)))
#             feat_v2 = F.relu(self.rgb_fc2(self.cnn_v2(rgb_v2)))
