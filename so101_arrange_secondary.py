...
@register_env("ArrangeCubeSO101-v1", max_episode_steps=1500)
class ArrangeCubeSO101Env(PickCubeSO101Env):
	...
	def __init__(self, *args, robot_uids="so101", **kwargs):
		...
		self.CENTER_POSITION_LEFT = np.array([0.121, 0.25, self.CUBE_HALF_SIZE])  # area2 左
		self.CENTER_POSITION = np.array([0.3, 0.25, self.CUBE_HALF_SIZE])          # area1 中
		self.CENTER_POSITION_RIGHT = np.array([0.3 * 2 - 0.121, 0.25, self.CUBE_HALF_SIZE]) # area0 右
		...
		self.CENTER_BL_OFFSET = np.array([-0.07, -0.07, 0.0])  # ✔️ BL 左下偏移 替换原BR右下
		self.AREA1_BL_POS = torch.tensor(self.CENTER_POSITION + self.CENTER_BL_OFFSET, dtype=torch.float32) # ✔️ 新BL点位
		...
		self.CLAMP_TIME = 3.0          
		self.RELEASE_TIME = 3.0        
		self.RESET_TIME = 4.0          

		self.ADD_TIME_TCP2AREA2 = 1.0
		self.ADD_TIME_AREA22BL = 4.0
		self.ADD_TIME_BL2AREA1 = 2.0
		self.ADD_TIME_AREA12AREA2 = 6.0

		self.arm_step_actions = [
			"move2area2", "clamp", "move2bl", "release_up", "move2area1", "clamp",
			"move2area2", "release_up", "move2bl", "clamp", "move2area1",
			"release_up", "reset"
		]
		
		self.arm_step_durations = torch.tensor([
			self.MOVE_TO_CUBE_TIME + self.ADD_TIME_TCP2AREA2,
			self.CLAMP_TIME,
			self.MOVE_WITH_CUBE_TIME + self.ADD_TIME_AREA22BL,
			self.RELEASE_TIME,
			self.MOVE_TO_CUBE_TIME + self.ADD_TIME_BL2AREA1,
			self.CLAMP_TIME,
			self.MOVE_WITH_CUBE_TIME + self.ADD_TIME_AREA12AREA2,
			self.RELEASE_TIME,
			self.MOVE_TO_CUBE_TIME + self.ADD_TIME_AREA22BL,
			self.CLAMP_TIME,
			self.MOVE_WITH_CUBE_TIME + self.ADD_TIME_BL2AREA1,
			self.RELEASE_TIME,
			self.RESET_TIME
		], dtype=torch.float32)
	...
	def _load_agent(self, options: dict):
		...
		self.one_agent = self.agent.agents[1]  # 核心修改：主机械臂切换为 agent1 (原副臂)
	...
	def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
		...
		self.AREA1_BL_POS = self.AREA1_BL_POS.to(device)
		
		with torch.device(self.device):
			self.agent.agents[0].robot.set_qpos((torch.rand((b, 6)) * 2 - 1) * 0.20 + torch.tensor(self.agent.agents[1].keyframes["rest"].qpos))
		...
		if not hasattr(self, "active_cube_idx"):
			...
			self.gripper_state = torch.zeros(self.num_envs, dtype=torch.long, device=device)
			self.arm_step = torch.zeros(self.num_envs, dtype=torch.long, device=device)
			self.step_in_phase = torch.zeros(self.num_envs, dtype=torch.long, device=device)
		...
		def random_rotate_quat():
			...
			qs[:, 0] = torch.cos(half_yaw)
			qs[:, 3] = torch.sin(half_yaw)
			return qs
		...
		self.oracle_indicator.set_pose(Pose.create_from_pq(p=self.DEFAULT_TCP_CENTER_POS_1))

	def step(self, action: Union[np.ndarray, torch.Tensor, dict]):
		# 主臂更改
		if isinstance(action, dict):
			raw_uid = self.agent.agents[1].uid
			arm_key = f"{raw_uid}-1"
			...
			for key in action.keys():
				if key != arm_key:
					...
					if isinstance(val, torch.Tensor):
						...
						action[key] = (np.random.rand_like(val) * 2 - 1) * 0.20
		...
		def move_without_cube(start_p: torch.Tensor, end_p: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
			s = 0.5 - 0.5 * torch.cos(t * torch.pi)
			xy = (1 - s)[:, None] * start_p[:, :2] + s[:, None] * end_p[:, :2]
			z = torch.full_like(t, self.IN_AIR_HEIGHT)
			return torch.cat([xy, z[:, None]], dim=-1).float()
		...
		# 核心点位替换: area2(左) 替换area0, BL 替换BR
		area2_pos = self.AREA_CENTERS[2].repeat(self.num_envs,1).float().to(device)
		area1_pos = self.AREA_CENTERS[1].repeat(self.num_envs,1).float().to(device)
		bl_pos = self.AREA1_BL_POS.repeat(self.num_envs,1).float().to(device)
		reset_pos = torch.from_numpy(self.DEFAULT_TCP_CENTER_POS_1).float().to(device).repeat(self.num_envs,1)

		step_action_map = [
			("no_cube", reset_pos, area2_pos, self.MOVE_TO_CUBE_TIME, 0),  # 0 移到 area2 center
			("clamp", area2_pos, area2_pos, self.CLAMP_TIME, 1),          # 1 在 area2 夹紧
			("with_cube", area2_pos, bl_pos, self.MOVE_WITH_CUBE_TIME, 1),# 2 area2 → BL
			("release_up", bl_pos, bl_pos, self.RELEASE_TIME, 0),         # 3 松开抬升
			("no_cube", bl_pos, area1_pos, self.MOVE_TO_CUBE_TIME, 0),    # 4 移到 area1 center
			("clamp", area1_pos, area1_pos, self.CLAMP_TIME, 1),          # 5 在 area1 夹紧
			("with_cube", area1_pos, area2_pos, self.MOVE_WITH_CUBE_TIME, 1), # 6 area1 → area2
			("release_up", area2_pos, area2_pos, self.RELEASE_TIME, 0),   # 7 松开抬升
			("no_cube", area2_pos, bl_pos, self.MOVE_TO_CUBE_TIME, 0),    # 8 移到 BL
			("clamp", bl_pos, bl_pos, self.CLAMP_TIME, 1),                # 9 在 BL 夹紧
			("with_cube", bl_pos, area1_pos, self.MOVE_WITH_CUBE_TIME, 1), #10 BL → area1
		...
		if not hasattr(self, "left_arm_rest_qpos"):
			...
			self.left_arm_rest_qpos = torch.from_numpy(rest_np).float().to(device)
	...
	def _get_obs_extra(self, info: dict):
		...
		# 主机械臂改为 agent1
		agent_main = self.agent.agents[1]
		agent_sub = self.agent.agents[0]
		is_grasp_cubeA = agent_main.is_grasping(self.cube_A)
		is_grasp_cubeB = agent_main.is_grasping(self.cube_B)
		is_grasp_cubeC = agent_main.is_grasping(self.cube_C)
		
		tcp_main = agent_main.tcp_pose.raw_pose
		tcp_sub = agent_sub.tcp_pose.raw_pose
		tcp_pose = torch.cat([tcp_main, tcp_sub], dim=-1)
		...
		if phase_mask.any():
			tcp_pos = agent_main.tcp_pose.p[phase_mask]
			perm = self.PERM_IDX_TO_ARR[self.cube_perm_idx[phase_mask]]
			phase_sub = phase_idx[phase_mask] // 3
			cube_ids = torch.where(phase_sub==1, perm[:,1], perm[:,2]) # ✔️ 核心规则: phase_sub=1→area1(perm1),其他→area2(perm2)
		...
		obs["diff_oracle_left"] = oracle_pos - tcp_main[..., :3]
	...
	def evaluate(self):
		...
		tcp_pos = agent_main.tcp_pose.p
		oracle_pos = self.oracle_indicator.pose.p
		dist_to_oracle = torch.linalg.norm(tcp_pos - oracle_pos, dim=1)
		
		tcp_quat = agent_main.tcp_pose.q
