...
@register_env("ArrangeCubeSO101Color-v0", max_episode_steps=4500)
class ArrangeCubeSO101Env(PickCubeSO101Env):
	...
	def set_active_arm(self, arm: int):
		assert arm in [0, 1]
		print("Switch to arm", arm)
		print(self.oracle_indicator.pose.p)
		self.active_arm = arm
		self.arm_step = torch.zeros_like(self.arm_step)
		self.step_in_phase = torch.zeros_like(self.step_in_phase)
		self.one_agent = self.agent.agents[arm]
		if arm == 0:
			self.arm_step_actions = [
				"move2area0", "clamp", "move2br", "release_up", "move2area1", "clamp",
				"move2area0", "release_up", "move2br", "clamp", "move2area1",
	...
	def _load_scene(self, options):
		...
		self.cube_A = build_cube_w_friction(
			self.scene, self.CUBE_HALF_SIZE, [0.40, 0.00, 0.6, 1.0], "cube_A",
			initial_pose=Pose.create_from_pq(p=[0.0, 0.0, 1.0], q=[1.0, 0.0, 0.0, 0.0])
		)
		self.cube_B = build_cube_w_friction(
			self.scene, self.CUBE_HALF_SIZE, [0, 1, 1, 1], "cube_B",
			initial_pose=Pose.create_from_pq(p=[0.0, 0.0, 2.0], q=[1.0, 0.0, 0.0, 0.0])
		)
		self.cube_C = build_cube_w_friction(
			self.scene, self.CUBE_HALF_SIZE, [1, 0, 1, 1], "cube_C",
	...
	def _get_obs_extra(self, info: dict):
		...
		# 1. 抓取状态检查 (保持你原有的逻辑)
		agent_left = self.agent.agents[self.active_arm]
		agent_right = self.agent.agents[1-self.active_arm]
		...
		if phase_mask.any():
			...
			cube_ids = torch.where(phase_sub==1, perm[:,1], perm[:,(0 if self.active_arm==0 else 2)]) # =1用perm[1]，0/2/3都用perm[0]
